...
class HMCustomBackgroundView: UIView {
    ...
    var listOfWords = ["Apple", "Doppelganger", "aforetime", "Yeet", "Adamant", "Serendipity", "Butterfly", "Tailor", "Rue", "Inconspicuous"]
    ...
    func initializePostiionOfWords() {
        for eachLabel in listOfLabels {
            ...
            NSLayoutConstraint.activate([
                eachLabel.topAnchor.constraint(equalTo: self.topAnchor, constant: randomNumber(max: 300)),
                eachLabel.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: -200)
            ])
        }
    }
    
    func animateWords() {
        for eachLabel in self.listOfLabels {
